...
static vector efield(GRETA_Siggen_Setup *setup, point pt, int_pt ipt);
static int    setup_efield(GRETA_Siggen_Setup *setup);
static int    setup_wp(GRETA_Siggen_Setup *setup);
static int    setup_velo(GRETA_Siggen_Setup *setup);
...
int field_setup(GRETA_Siggen_Setup *setup) {
  ...
  tell(NORMAL, "Crystal axes will be rotated for mobilities by %d degrees\n",
       setup->xtal_axis_rotation);
  tell(NORMAL, "Detector temperature is set to %.1f K\n", setup->xtal_temp);

  tell(NORMAL, "Total number of segments: %d\n", TOT_NSEGMENTS);
  if (TOT_NSEGMENTS != nsegs) {
    error("Error: Number of segments read from file = %d != %d\n",
	  TOT_NSEGMENTS, nsegs);
    return -1;
  }
  ...
  if (setup->xtal_type < 0 || setup->xtal_type >= N_CRYSTAL_TYPES) {
    error("Invalid crystal type %d\n", setup->xtal_type);
    return -1;
  }
...
static int efield_exists(GRETA_Siggen_Setup *setup, point pt) {
  char ptstr[MAX_LINE];
  int i, j, k;
  int ix, iy, iz;
  int_pt ipt;
...
int wpotentials(GRETA_Siggen_Setup *setup, point pt, float *wp) {
  ...
  for (segno = 0; segno < TOT_NSEGMENTS; segno++) {
    wp[segno] = 0.0;
    for (i = 0; i < 2; i++) {
      for (j = 0; j < 2; j++) {
	for (k = 0; k < 2; k++) {
	  wp[segno] += w[i][j][k] * setup->wpot[ipt.x+i][ipt.y+j][ipt.z+k][segno];
	}
      }
    }
  }
  /*Added by Karin -- renormalize WP when extrapolating*/
  if (res > 0) {
    sum = 0;
    for (segno = 0; segno < TOT_NSEGMENTS; segno++) {
      if (wp[segno] < 0.0) 
    ...
    if (sum != 0) {
      for (segno = 0; segno < TOT_NSEGMENTS; segno++) {
	wp[segno] /= sum; // Ensure WP adds up to 1
      }    
    }else {
      for (segno = 0; segno <TOT_NSEGMENTS; segno++)
	wp[segno] = 0;
...
*/
int drift_velocity(GRETA_Siggen_Setup *setup, point pt, float q, vector *velo) {
  vector e, en;
  int_pt ipt;
  int    i;
  float  abse;
  ...
  en = vector_rotate_z(en, -setup->xtal_axis_rotation);
...
#define POW6(x) ((x)*(x)*(x)*(x)*(x)*(x))
  ...
  v = *velo;
  v = vector_rotate_z(v, setup->xtal_axis_rotation);
  velo->x = v.x;
  velo->y = v.y;
  velo->z = v.z;
...
static int setup_wp(GRETA_Siggen_Setup *setup)
{
  ...
  if (wpot == NULL) {//assuming setup->numx, setup->numy, setup->numz never change as for setup_efld
	...
	for (k = 0; k < setup->numz; k++) {
	  if ((wpot[i][j][k] = malloc(nsegments*sizeof(*wpot[i][j][k]))) == NULL) {
	    error("Malloc failed in setup_wp\n");
	    // memory leak again
            return 1;
	  }
  ...
  for (i = 0; i < setup->numx; i++) {
    for (j = 0; j < setup->numy; j++) {
      for (k = 0; k < setup->numz; k++) {
        for (segno = 0; segno < nsegments; segno++) {
          wpot[i][j][k][segno] = 0.0;
        }
      }
    }
  }
  ...
  tell(NORMAL, "Done reading weighting potentials\n");
  ...
  if (!strstr(setup->wp_name, "unf") && (fp = fopen(fname_unf, "w"))) {
    ...
    tell(NORMAL, "Done saving %d weighting potentials\n", nsegments);
    fclose(fp);
  }
  return 0;
}
