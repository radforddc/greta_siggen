...
def main():
    ...
    x = set(data[:,1])  # sets only have one copy of anything, so if there are repeats, they are removed
    y = set(data[:,2])
    z = data[:,z_index]
    ...
    # label axes
    plt.xlabel("y position [mm]", size=13)
    plt.ylabel("z position [mm]", labelpad=8,  size=13)

    # make the color legend
    cbar = plt.colorbar(ip, fraction=0.046, pad=0.04)
    label_text = ["x-field", "y-field", "z-field", "Field", "Potential"]
    units_text = [" [V/cm]", " [V/cm]", " [V/cm]", " [V/cm]", " [V]"]
    cbar.set_label(label_text[z_index-3] + units_text[z_index-3], size=13, labelpad=8)
    cbar.set_label(label_text[z_index-3] + units_text[z_index-3], size=13, labelpad=8)
    plt.title(label_text[z_index-3] + " vs. Position\n", fontsize=16, linespacing=0.4)
